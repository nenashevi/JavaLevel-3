...
public class Controller implements Initializable {
    ...
    private File msgHistoryFile;
    private String msgHistoryFileName;
    List<String> msgHistoryLines;
    private int lastRecordsNumber = 3;
//    private BufferedWriter writer;
    ...
    public void createMsgFile() {
        ...
        msgHistoryFileName = String.format("history_%s.txt", loginField.getText().trim());
        //msgHistoryFileName = "history_login.txt";
        msgHistoryFile = new File(msgHistoryFileName);
    ...
    public void setLastMsgToTextArea() {
        //textArea.appendText("test message \n");
        try {
            reader = new BufferedReader(new FileReader(msgHistoryFile));
            String s;
            msgHistoryLines = new ArrayList<>();
            while ((s = reader.readLine()) != null) {
                msgHistoryLines.add(s);
            }
        ...
        int startHistoryLines = msgHistoryLines.size() - lastRecordsNumber;
        if (startHistoryLines < 0) {
            startHistoryLines = 0;
        }
        for (int i = startHistoryLines; i < msgHistoryLines.size(); i++) {
            textArea.appendText(msgHistoryLines.get(i) + "\n");
        }
    ...
    public  void writeMsgToFile(String msg) {
        ...
        try {
            //writer = new BufferedWriter(new FileWriter(msgHistoryFile));
            writer = new FileWriter(msgHistoryFile, true);
            //writer.write(msg + "\n");
            writer.write(msg + "\n");
        } catch (IOException e) {
            e.printStackTrace();
